...
namespace Visyde
{
    ...
    public class LobbyBrowserUI : MonoBehaviour
    {
        ...
        [Inject]
        private void Construct(Connector connector)
        {
            _connector = connector;
        }

        void OnEnable(){
            _connector.onRoomListChange += onRoomListUpdate;
            _connector.onCreateRoomFailed += onCreateRoomFailed;
            _connector.onJoinRoom += OnJoinedRoom;
            _connector.onLeaveRoom += OnLeftRoom;
            _connector.onPlayerJoin += OnPlayerJoined;
            _connector.onPlayerLeave += OnPlayerLeft;
        }
        void OnDisable(){
            _connector.onRoomListChange -= onRoomListUpdate;
            _connector.onCreateRoomFailed -= onCreateRoomFailed;
